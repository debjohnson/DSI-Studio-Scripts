...
function varargout = roi_pairs(varargin)
	...
	% Last Modified by GUIDE v2.5 17-Jan-2012 13:21:35
...
function dsi_studio_button_Callback(hObject, eventdata, handles)
	...
	hMainGui = getappdata(0, 'hMainGui');
		% * TIP: This line of code is at the beginning of almost every function; 
	...
	% When user clicks button, open file browser window (uigetfile); Limit selectable files to those that have a .exe extension (*.exe)
	% After user selects DSI Studio, set variables [dsi_studio, dsi_studio_path] will be set to the file name and path, respectively
	...
	% Store the dsi_studio_pointer variable in the GUI.
		% * TIP: Without this line of code, the dsi_studio_pointer variable will not be accessible from any other functions
...
% ------ See dsi_studio_button_Callback function for comments; structure of function below is identical ------ %
...
% hObject    handle to save_params_button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
